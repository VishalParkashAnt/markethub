		...
		*/
        ...
        function discountProducts($offset = null, $limit = null, $layout = null)
        {
			...
			foreach($getDiscounts as $key=>$valspro){
				...
				$getitems = $itemstable->find()->contain('Photos')->contain('Forexrates')->contain('Users')->contain('Shops')->where(['Items.status'=>'publish','Items.id'=>$valspro['id']])->first();
        ...
        {	
        	...
        	$itemstable = TableRegistry::get('Items');
        	$items_data = $itemstable->find()->contain('Photos')->contain('Forexrates')->contain('Users')->contain('Shops')->where(['Items.status'=>'publish','Items.category_id'=>$category])->offset($offset)->limit($limit)->order(['Items.id DESC'])->all();
