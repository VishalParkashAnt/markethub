...
class ApiController extends AppController
{
	...
	public function beforeFilter(Event $event)
	{
        ...
        function home()
        {
        	...
        	$featured_category_detail = $this->featuredcategory();
		...
		echo '{"status":"true","ascending_order":'.$orders.',"layout_design": '.json_encode($re1array).',"banner": ' . $banner_detail . ',"category": ' . $category_detail . ',"featuredcategory": ' . $featured_category_detail . ',"daily_deals": {"valid_till": ' . $tdy . ', "items":' . $daily_deals . ', "valid_time":' . time() . '},"item_lists":' . json_encode($data).'}';
        	die;
        }
	...
	function featuredcategory()
	{
		...
		$this->loadModel('Categories');
		$resultarray = array();
		$CategoryModel = $this->Categories->find()->where(['category_parent' => 0])->where(['featured' => 1])->toArray();//all',array('conditions'=>array('category_parent'=>'0')));
		if (count($CategoryModel) > 0) {
			for ($i = 0; $i < count($CategoryModel); $i++) {
				$resultarray[$i] = array();
				$categoryId = $CategoryModel[$i]['id'];
