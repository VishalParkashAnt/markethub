        ...
        echo  '<div class="table-responsive m-t-10">';
          echo '<table id="shippedorderstable" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">';
            echo '<thead>';
							...
							{
                //echo '<pre>'; print_r($getitemuser->toArray()); die;
                    ...
                    if($user_det['totalcost'] == 0)
                    {
                        $uer_paid_amount = $user_det['discount_amount'];
                    }else{
                        $uer_paid_amount = $user_det['totalcost']-$user_det['admin_commission'];
                    }

										echo '<td class="invoicePayMthd aligncenter">'.$uer_paid_amount.'</td>';
