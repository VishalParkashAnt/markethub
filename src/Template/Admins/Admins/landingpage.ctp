...
//echo $homepagesettings->layout;
      ...
      if(empty($homepagesettings->layout)){
       ...
       echo '<div class="row">
        ...
        echo $this->Form->input('Layout', array('options' => $options2,'default'=>$layoutstatus,
            ...
            '$(".customslider").slideToggle();$(".layoutselection").slideToggle(); $(".widgetsselector").slideToggle();'));
        echo '</div></div></div>';

			$availwidgets = array(__d('admin', 'Recently Added'),
								__d('admin', 'Most Popular'), 
								__d('admin', 'Top Stores'),
             					__d('admin', 'Featured Items'),
             					__d('admin', 'Discounts'),
             					// __d('admin', 'Suggested Items'),
     ...
     ?>
		...
		<div class="layoutselection" style="<?php echo $displaydiv; ?>">
			<div class="">
			<?php
				$layoutArray = array('slider1'=>'layout1',
					'slider2'=>'layout2',
					'slider3'=>'layout3',
					'slider4'=>'layout4',
					'slider5'=>'layout5'
				...
				$widgetsett = json_decode($homepagesettings->widget_settings);
				$layout_categories = $homepagesettings->categories;
			?>
				<span><?php echo __d('admin', 'Popular Items'); ?>: </span>

				<?php
				foreach($layoutArray as $eachkey=>$eachval)
						{
							//echo $widgetsett->most_popular;
							//echo 'sss<br/>';
							//echo $eachkey;
							//die;
						}
				?>
				<select name="slider[most_popular]" class="form-control" id="most_popular">
					<?php
						foreach($layoutArray as $eachkey=>$eachval)
						{
							$setback = ($widgetsett->most_popular == $eachkey) ? 'selected="selected"' : '';
							?>
							<option value="<?php echo $eachkey; ?>" <?php echo $setback; ?>><?php echo $eachval; ?></option>
							<?php
						}
					?>
				</select>
			...
			<div class="">
				<span><?php echo __d('admin', 'Featured Items'); ?>: </span>
				<select class="form-control" name="slider[featured_items]" id="featured_items">
					<?php
						foreach($layoutArray as $eachkey=>$eachval)
						{
							$setback = ($widgetsett->featured_items == $eachkey) ? 'selected="selected"' : '';
							?>
							<option value="<?php echo $eachkey; ?>" <?php echo $setback; ?>><?php echo $eachval; ?></option>
							<?php
						}
			...
			<div class="">
				<span><?php echo __d('admin', 'Discount Items'); ?>: </span>
				<select class="form-control" name="slider[discounts]" id="discounts">
					<?php
						foreach($layoutArray as $eachkey=>$eachval)
						{
							$setback = ($widgetsett->discounts == $eachkey) ? 'selected="selected"' : '';
							?>
							<option value="<?php echo $eachkey; ?>" <?php echo $setback; ?>><?php echo $eachval; ?></option>
							<?php
						}
			...
			<div class="">
				<span><?php echo __d('admin', 'Top Rated Items'); ?>: </span>
				<select class="form-control" name="slider[top_rated]" id="top_rated">
					<?php
						foreach($layoutArray as $eachkey=>$eachval)
						{
							$setback = ($widgetsett->top_rated == $eachkey) ? 'selected="selected"' : '';
							?>
							<option value="<?php echo $eachkey; ?>" <?php echo $setback; ?>><?php echo $eachval; ?></option>
							<?php
						}
			...
			<div class="">
				<span><?php echo __d('admin', 'Recently Added Items'); ?>: </span>
				<select class="form-control" name="slider[recently_added]" id="recently_added">
					<?php
						foreach($layoutArray as $eachkey=>$eachval)
						{
							$setback = ($widgetsett->recently_added == $eachkey) ? 'selected="selected"' : '';
							?>
							<option value="<?php echo $eachkey; ?>" <?php echo $setback; ?>><?php echo $eachval; ?></option>
							<?php
						}
