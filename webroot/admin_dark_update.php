	...
	if(isset($ftmp)) {   
		if(!in_array($ext, $extensionarray)) {
			...
			$errmsg = 'Please upload image upto 10MB for Site logo';
				...
				$originalImage = $ftmp; 
				$outputImage = $finalPath."darklogo.png";      
				$quality = 9;
				...
				<script>
					...
					par.getElementById('show_dark_url').src = '<?php echo $img_src; ?>';
					par.getElementById('image_dark').value = '<?php echo $newname; ?>';	
					var imagename = par.getElementById('image_dark').value;
					if(imagename){
						parent.document.getElementById('removedarkimg').style.display='block';
					}
				</script>
				<?php
		} else {
...
parent.document.getElementById('sitedarklogoerr').style.display = 'block';
parent.document.getElementById('sitedarklogoerr').innerHTML = '<?php echo $errmsg; ?>';
setTimeout(function() {
			 parent.document.getElementById('sitedarklogoerr').innerHTML = '';
			}, 5000);
</script>
