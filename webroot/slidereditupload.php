...
<?php
	...
	if(isset($ftmp)) {   
		if(!in_array($ext, $extensionarray)) {
			...
			echo "<script>parent.document.getElementById('sliderimguploaderr').style.display = 'block'; parent.document.getElementById('sliderimguploaderr').innerHTML = '".$errmsg."'; setTimeout(function() { parent.document.getElementById('sliderimguploaderr').innerHTML = ''; }, 5000);</script>"; 
		} elseif($appImageValues[0] < 1024 || $appImageValues[1] < 370) {   
			// $appImageValues[0] is the width & $appImageValues[1] is the height.
			$errmsg = "Error : image size greater than 1024 x 370 pixels.";
			echo "<script>parent.document.getElementById('sliderimguploaderr').style.display = 'block'; parent.document.getElementById('sliderimguploaderr').innerHTML = '".$errmsg."'; setTimeout(function() { parent.document.getElementById('sliderimguploaderr').innerHTML = ''; }, 5000);</script>";  
		}  elseif ($appImageValues[0] >= 1024 && $appImageValues[1] >= 370 && count($appImageValues) >= 6 && in_array($ext, $extensionarray) && (end($appImageValues) == "image/jpeg" || end($appImageValues) == "image/png")) { 
			...
			if(empty($result)) {
				$errmsg = "Error : There was an error moving the uploaded file.";
				echo "<script>parent.document.getElementById('sliderimguploaderr').style.display = 'block'; parent.document.getElementById('sliderimguploaderr').innerHTML = '".$errmsg."'; setTimeout(function() { parent.document.getElementById('sliderimguploaderr').innerHTML = ''; }, 5000);</script>";
			}
